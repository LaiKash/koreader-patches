...
ReaderView.paintTo = function(self, bb, x, y)
    ...
    local left_corner_header = string.format("%s %s %s", book_chapter, separator.bullet, book_title)
    local right_corner_header = string.format("%s %s %s", time, separator.bullet, battery)
    ...
    header:paintTo(bb, x, y)
	    ...
	    -- ===========================!!!!!!!!!!!!!!!=========================== -
    -- Permanent Footer (bottom of screen)
    -- Left: "page / total pages"
    -- Right: "battery  |  clock"
    -- ===========================!!!!!!!!!!!!!!!=========================== -

    local footer_font_face = header_font_face
    local footer_font_size = 12
    local footer_font_bold = true
    local footer_font_color = header_font_color

    local footer_bottom_padding = 0  -- space from bottom edge
    local footer_side_padding = 20    -- space from left/right screen edges
    local footer_text_padding = 4

	  -- Compute pages/time left in chapter
    local minutes_left = ""
    if self.ui.toc and pages_left > 0 then
        -- Estimate reading time (simple average: 1 page = 1 min)
        local avg_time_per_page = 60  -- seconds per page; adjust if desired
        local seconds_left = pages_left * avg_time_per_page
        local mins = math.floor(seconds_left / 60)
        local hrs = math.floor(mins / 60)
        mins = mins % 60
        if hrs > 0 then
            minutes_left = string.format("%d hrs %d min left", hrs, mins)
        else
            minutes_left = string.format("%d min left", mins)
        end
    end

    -- Prepare footer text
    local inclusive_pages_left = pages_left + 1 --inclusive of the current page
    local footer_left_text  = ""
    local footer_right_text = percentage_format
    if inclusive_pages_left > 1 then
        footer_left_text = string.format("%d pages left in chapter", inclusive_pages_left)
    else
        footer_left_text = string.format("%d page left in chapter", inclusive_pages_left)
    end

    -- Create text widgets
    local left_footer_text = TextWidget:new{
        text = footer_left_text,
        face = Font:getFace(footer_font_face, footer_font_size),
        bold = footer_font_bold,
        fgcolor = footer_font_color,
        padding = footer_text_padding,
    }
    local right_footer_text = TextWidget:new{
    ...
    local dynamic_space_footer = screen_width
        - left_footer_text:getSize().w
        - right_footer_text:getSize().w
        - (footer_side_padding * 2)

    -- Footer Y-position (bottom of screen)
    local footer_height = math.max(left_footer_text:getSize().h, right_footer_text:getSize().h)
    ...
    local footer = CenterContainer:new{
        ...
        VerticalGroup:new{
            HorizontalGroup:new{
                HorizontalSpan:new{ width = footer_side_padding },
                left_footer_text,
                HorizontalSpan:new{ width = dynamic_space_footer },
                right_footer_text,
                HorizontalSpan:new{ width = footer_side_padding },
            },
        },
